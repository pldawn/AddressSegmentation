...
def viterbi_decode_with_conditions(score, transition_params, conditions):
  """Decode the highest scoring sequence of tags outside of TensorFlow with conditions.

  This shouldonly be used at test time.

  Args:
    score: A [seq_len, num_tags] matrix of unary potentials.
    transition_params: A [num_tags, num_tags] matrix of binary potentials.
    conditions: A [seq_len] array of tag indices.
  ...
  for t in range(1, score.shape[0]):
    v = np.expand_dims(trellis[t - 1], 1) + transition_params
    fixed = conditions[t - 1]
    if fixed < 0:
      trellis[t] = score[t] + np.max(v, 0)
      backpointers[t] = np.argmax(v, 0)
    else:
      trellis[t] = score[t] + v[fixed]
      backpointers[t] = fixed

  fixed_tail = conditions[-1]
  if fixed_tail < 0:
    viterbi = [np.argmax(trellis[-1])]
  else:
    viterbi = [fixed_tail]
  ...
  viterbi_score = np.max(trellis[-1]) if fixed_tail < 0 else trellis[-1][fixed_tail]
  return viterbi, viterbi_score
...
class CrfDecodeForwardRnnCell(rnn_cell.RNNCell):
  ...
  def __init__(self, transition_params, conditions=None):
    ...
    Args:
      ...
      conditions: A [batch_size, max_time] matrix. [B, T] not [B, T-1]
    """
    self._transition_params = array_ops.expand_dims(transition_params, 0)
    self._num_tags = transition_params.get_shape()[0].value
    self._conditions = None
    if conditions is not None:
      self._conditions = array_ops.transpose(conditions)  # [B, T] -> [T, B]
  ...
  def __call__(self, inputs, state, scope=None, time=0):
    ...
    if self._conditions is None:
      ...
      backpointers = math_ops.argmax(transition_scores, 1)  # [B, O]
      backpointers = math_ops.cast(backpointers, dtype=dtypes.int32)
    else:
      cond = self._conditions[time]
      batch = array_ops.constant(0, dtype=dtypes.int32)
      batch_size = self._conditions.get_shape()[1]
      time_size = self._conditions.get_shape()[0].value
      tag_size = state.get_shape()[1].value
      max_tran_scores = tensor_array_ops.TensorArray(dtype=dtypes.float32,
                                                     size=batch_size,
                                                     element_shape=tensor_shape.TensorShape([tag_size]),
                                                     tensor_array_name='max_tran_scores_ta')
      backpointers = tensor_array_ops.TensorArray(dtype=dtypes.int32,
                                                  size=batch_size,
                                                  element_shape=tensor_shape.TensorShape([tag_size]),
                                                  tensor_array_name='backpointers_ta')

      def _batch_step(batch, max_tran_scores, backpointers):
        tran_tmp = control_flow_ops.cond(
          cond[batch] <= -1,  # condition
          lambda: math_ops.cast(math_ops.reduce_max(transition_scores[batch], 0), dtypes.float32),  # true_fn, [O]
          lambda: math_ops.cast(transition_scores[batch][cond[batch]], dtypes.float32))  # false_fn, [O]
        back_tmp = control_flow_ops.cond(
          cond[batch] <= -1,  # condition
          lambda: math_ops.cast(math_ops.argmax(transition_scores[batch], 0), dtypes.int32),  # true_fn, [O]
          lambda: math_ops.cast(array_ops.zeros_like([0 for _ in range(tag_size)]) + cond[batch], dtypes.int32))  # false_fn, [O]

        max_tran_scores = max_tran_scores.write(batch, tran_tmp)
        backpointers = backpointers.write(batch, back_tmp)
        batch += 1
        return (batch, max_tran_scores, backpointers)

      _, max_tran_scores, backpointers = control_flow_ops.while_loop(
        cond=lambda batch, *_: batch < batch_size.value,
        body=_batch_step,
        loop_vars=(batch, max_tran_scores, backpointers),
        parallel_iterations=8,
        swap_memory=True)

      new_state = math_ops.cast(inputs + max_tran_scores.stack(), dtype=dtypes.float32)  # [B, O]
      backpointers = backpointers.stack()  # [B, O]
...
def crf_decode_with_conditions(potentials, transition_params, sequence_length, conditions=None):
  """Decode the highest scoring sequence of tags with conditions in TensorFlow.
  ...
  Args:
    ...
    transition_params: A [num_tags, num_tags] matrix of
              binary potentials.
    sequence_length: A [batch_size] vector of true sequence lengths.
    conditions: A [batch_size, max_seq_len] matrix of tag indices.
  ...
  Returns:
    ...
    best_score: A [batch_size] vector, containing the score of `decode_tags`.
  """
  # if conditions is None, we create a default tensor, its shape is same with
  # potentials.shape[:-1] and values are all 0.
  if conditions is None:
    conditions = tf.cast(tf.reduce_mean(tf.ones_like(potentials) * -1, -1), dtypes.int32)  # [B, T]
  ...
  def _multi_seq_fn():
    ...
    # Computes forward decoding. Get last score and backpointers.
    crf_fwd_cell = CrfDecodeForwardRnnCell(transition_params, conditions=conditions)
    initial_state = array_ops.slice(potentials, [0, 0, 0], [-1, 1, -1])
    initial_state = array_ops.squeeze(initial_state, axis=[1])  # [B, O]
    inputs = array_ops.slice(potentials, [0, 1, 0], [-1, -1, -1])  # [B, T-1, O]
    backpointers, last_score = rnn.dynamic_rnn_with_conditions(  # [B, T - 1, O], [B, O]
        crf_fwd_cell,
        inputs=inputs,
        sequence_length=sequence_length - 1,
        initial_state=initial_state,
        ...
        dtype=dtypes.int32)
    backpointers = gen_array_ops.reverse_sequence(  # [B, T - 1, O]
        backpointers, sequence_length - 1, seq_dim=1, name='backpointers')

    # Computes backward decoding. Extract tag indices from backpointers.
    crf_bwd_cell = CrfDecodeBackwardRnnCell(num_tags)
    initial_state = math_ops.cast(math_ops.argmax(last_score, axis=1),  # [B]
                                  dtype=dtypes.int32)
    last_conditions = conditions[:, -1]  # [B]
    initial_state = array_ops.where(last_conditions <= -1,
                                    initial_state, last_conditions, name='last_state')

    initial_state = array_ops.expand_dims(initial_state, axis=-1)  # [B, 1]
    decode_tags, _ = rnn.dynamic_rnn_with_conditions(  # [B, T - 1, 1]
        crf_bwd_cell,
        inputs=backpointers,
        sequence_length=sequence_length - 1,
        initial_state=initial_state,
